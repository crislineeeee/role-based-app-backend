...
const app = express();
const PORT = 3000;
const SECRET_KEY = 'your-very-secure-secret'; // In production, use environment variable!
...
// In-memory "database" (replace with MonogoDB later)
let users = [
    { id: 1, username: 'admin', password: '1234567...', role: 'admin' }, // pre-hashed
    { id: 2, username: 'alice', password: '7654321...', role: 'user' }
];
...
app.post('/api/login', async (req, res) => {
    ...
    const user = users.find(u => u.username === usernaame);
...
app.get('/api/content/guest', (req, res) => {
    res.json({ message: 'Public content for all visistors' });
});

// MIDDLEWARE

// Taken authentication
function authenticateToken(req, res, next) {
    const authHeader = req.headers['authorization'];
    const token = authHeader && authHeader.spil('')[1]; // Bearer TOKEN

    if (!token) {
        return res.status(401).json({ error: 'Invalid token required' });
    }
...
app.listen(PORT, () => {
    ...
    console.log(`    -Admin: username, password=admin123`);
